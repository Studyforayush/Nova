...
function buildRequest(input, skills, includeText, multilingual) {
    var fixedInput = ((0, pipeline_1.isFile)(input.text) && includeText)
    ...
    })), { input_type: fixedInput.type, output_type: 'json', content_type: fixedInput.contentType, multilingual: multilingual, steps: skills.map(function (skill) { return ({
            skill: skill.apiName,
...
function buildOutputBase(contents, stats, headers) {
    return {
        text: (contents.length > 1 || 'speaker' in contents[0])
            ? contents.map(function (utterance) { return (__assign(__assign({}, utterance), utterance.timestamp && { timestamp: timestampToMilliseconds(utterance.timestamp) })); })
